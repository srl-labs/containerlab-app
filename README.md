...
Web and desktop hosts for [containerlab](https://containerlab.dev/) built on top of [`@srl-labs/clab-ui`](https://github.com/srl-labs/clab-ui). They provide a UI for exploring, opening, deploying, destroying, and redeploying containerlab topologies through a reachable `clab-api-server`.

The production web image serves the UI, authenticates against `clab-api-server`, proxies topology and lifecycle operations, and streams runtime events into the browser. Neither the web host nor the desktop host starts or manages `clab-api-server`; run that service separately.
...
![screenshot](https://raw.githubusercontent.com/srl-labs/containerlab-web/refs/heads/main/apps/web/resources/screenshot.png)
...
- A reachable `clab-api-server` endpoint, commonly `https://localhost:8090` on the host
...
| `CLAB_API_URL` | `https://localhost:8090` | Default `clab-api-server` endpoint shown to the UI |
...
| `CONTAINERLAB_WEB_CLIENT_ROOT` | auto-detected | Override the built web client directory used by web/desktop app-server startup |
...
On Linux, the desktop launcher forces Electron onto the X11/Ozone backend so it starts reliably under Ubuntu Wayland sessions. The packaged AppImage is written to `apps/desktop/release/Containerlab-0.0.1.AppImage`.
