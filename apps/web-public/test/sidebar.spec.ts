...
test("only the hovered rail item animates, without resizing the rail or editor", async ({ page }) => {
  ...
  await page.getByText("motion.clab.yml", { exact: true }).dblclick();
  ...
  expect(samples.every(({ width }) => width === samples.at(-1)!.width)).toBe(true);
  ...
  await expect(rail.getByRole("button", { name: "File Explorer", exact: true })).toHaveCSS("width", "36px");
  ...
  await page.getByRole("button", { name: "Pin rail", exact: true }).click();
  await expect(rail).toHaveCSS("width", "196px");
  await page.getByRole("button", { name: "Unpin rail", exact: true }).click();
  await expect(rail).toHaveCSS("width", "48px");
  await expect(page.locator(".react-flow__node")).toHaveCount(2);
  expect(await editor.boundingBox()).toEqual(bounds);
});
...
test("hover labels fit their text even when page JavaScript cannot run", async ({ page }) => {
  ...
  try {
    for (const label of ["Labs", "File Explorer", "Help & Feedback", "Settings", "Pin rail"]) {
...
test("pinning survives reload, unpins under the pointer and adapts to a narrow window", async ({ page }) => {
  ...
  await expect(rail).toHaveCSS("width", "196px");
  expect((await sidebar.boundingBox())!.width).toBe(initial.width + 148);
  await page.reload();
  await expect(rail.getByRole("button", { name: "Unpin rail", exact: true })).toHaveAttribute("aria-pressed", "true");
  ...
  await page.reload();
  await expect(rail.getByRole("button", { name: "Pin rail", exact: true })).toHaveAttribute("aria-pressed", "false");
});
...
test("carved labels fit each item and keep the surrounding explorer clickable", async ({ page }) => {
  ...
  const rail = page.getByTestId("workspace-rail");
  const labs = rail.getByRole("button", { name: "Labs", exact: true });
  const files = rail.getByRole("button", { name: "File Explorer", exact: true });
  await labs.hover();
  const labsWidth = (await labs.boundingBox())!.width;
  await files.hover();
  await expect(labs).toHaveCSS("width", "36px");
  const filesWidth = (await files.boundingBox())!.width;
  expect(filesWidth).toBeGreaterThan(labsWidth);
  await expect(rail).toHaveCSS("width", "48px");
...
test("closing the explorer preserves its filter and reopening works from the keyboard", async ({ page }) => {
  await page.goto("/");
  const rail = page.getByTestId("workspace-rail");
  const filter = page.getByPlaceholder("Filter labs, nodes, interfaces");
  await filter.fill("remember-this-filter");
  await page.getByRole("button", { name: "Close explorer", exact: true }).click();
  await expect(filter).toBeHidden();
  const labs = rail.getByRole("button", { name: "Labs", exact: true });
  await expect(labs).toBeFocused();
  await page.keyboard.press("Enter");
  await expect(filter).toHaveValue("remember-this-filter");
  await page.keyboard.press("Tab");
  const files = rail.getByRole("button", { name: "File Explorer", exact: true });
  await expect(files).toBeFocused();
  await expect(files.getByTestId("rail-item-surface")).toHaveCSS("opacity", "1");
  await expect(rail).toHaveCSS("width", "48px");
  await page.keyboard.press("Escape");
  await expect(files).toHaveCSS("width", "36px");
  await expect(rail).toHaveCSS("width", "48px");
  await expect(rail.getByRole("button", { name: "File Explorer", exact: true })).toBeFocused();
});
...
test("rail honors reduced motion and remains usable when storage is blocked", async ({ page }) => {
  ...
  await page.goto("/");
  const rail = page.getByTestId("workspace-rail");
  await expect(rail.getByRole("button", { name: "Pin rail", exact: true })).toHaveCSS("transition-duration", "0s");
  await rail.getByRole("button", { name: "Pin rail", exact: true }).click();
  await expect(rail).toHaveCSS("width", "196px");
  await rail.getByRole("button", { name: "Unpin rail", exact: true }).click();
  await expect(rail).toHaveCSS("width", "48px");
