...
export function RailLogo(props: { showTooltip?: boolean; side?: "left" | "right" }) {
  ...
  useEffect(() => {
    ...
    loader.load(publicAssetUrl("model.gltf"), (gltf) => {
      ...
      model.traverse((child) => {
        ...
        (child as THREE.Mesh).material = index === 0 ? liquid : body;
        index += 1;
      });
  ...
  return (
    ...
    >
      <IconButton
        size="small"
        onPointerDown={() => kickRef.current(Math.random() < 0.4)}
        aria-label="TopoViewer"
        data-testid="sandbox-sidebar-logo"
        sx={{ width: 36, height: 36, borderRadius: 1, overflow: "hidden" }}
      >
        <Box ref={hostRef} sx={{ width: 28, height: 28 }} />
      </IconButton>
    </Tooltip>
  );
}
