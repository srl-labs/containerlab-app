...
function assertAnnotationsContainNetworkNodes(
  ...
  for (const { id: networkId, type } of createdLinkBased) {
    const ann =
      annotations.networkNodeAnnotations?.find((n: any) => n.id === networkId) ??
      annotations.nodeAnnotations?.find((n: any) => n.id === networkId);
    expect(ann).toBeDefined();
...
test.describe("Network Node Position Persistence", () => {
  ...
  test("host network position persists after reload", async ({ page, topoViewerPage }) => {
    ...
    let reloadedHostId = networkIds.find((id) => id === hostId || id.startsWith("host:"));
    expect(reloadedHostId).toBeDefined();

    const posAfterFirstReload = await topoViewerPage.getNodePosition(reloadedHostId!);
    expect(Number.isFinite(posAfterFirstReload.x)).toBe(true);
    expect(Number.isFinite(posAfterFirstReload.y)).toBe(true);

    // Second reload should preserve the reloaded position.
    await topoViewerPage.gotoFile(EMPTY_FILE);
    await topoViewerPage.waitForCanvasReady();

    networkIds = await topoViewerPage.getNetworkNodeIds();
    reloadedHostId =
      networkIds.find((id) => id === reloadedHostId) ?? networkIds.find((id) => id.startsWith("host:"));
    expect(reloadedHostId).toBeDefined();

    const posAfterSecondReload = await topoViewerPage.getNodePosition(reloadedHostId!);
    expect(Math.abs(posAfterSecondReload.x - posAfterFirstReload.x)).toBeLessThan(80);
    expect(Math.abs(posAfterSecondReload.y - posAfterFirstReload.y)).toBeLessThan(80);
  });
...
test.describe("Network Node Modification", () => {
  ...
  test("editing host interface updates YAML and node id", async ({ page, topoViewerPage }) => {
    ...
    await expect
      ...
      .toEqual(expect.arrayContaining([expect.stringMatching(/^host:/)]));
    const networkIds = await topoViewerPage.getNetworkNodeIds();
    expect(networkIds.some((id) => id === newHostId || id === hostId || id.startsWith("host:eth1"))).toBe(true);

    const annotations = await topoViewerPage.getAnnotationsFromFile(EMPTY_FILE);
    const networkAnnotationIds = annotations.networkNodeAnnotations?.map((n: any) => n.id) ?? [];
    if (networkAnnotationIds.length > 0) {
      expect(
        networkAnnotationIds.some(
          (id: string) => id === newHostId || id === hostId || id.startsWith("host:eth1")
        )
      ).toBe(true);
    }
