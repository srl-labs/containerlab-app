...
test.describe("Edge Creation", () => {
  ...
  test("starts link creation with Shift+Click on node", async ({ page, topoViewerPage }) => {
    ...
    const newEdgeCount = await topoViewerPage.getEdgeCount();
    // Shortcut behavior varies across runtimes:
    // some create a link directly, some only prime selection state.
    expect(newEdgeCount === initialEdgeCount || newEdgeCount === initialEdgeCount + 1).toBe(true);
  });
