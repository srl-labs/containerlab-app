import "../../types/assets";
import { useWorkspaceHost, useWorkspaceLabs } from "../WorkspaceHost";
...
import type { TerminalPreferences } from "../state/runtimeTerminalSettings";

import type { ContainerState, LabState } from "../types";
...
} from "../state/runtimeTerminalActions";
import {
  resolveTerminalFontShortcutAction,
  resolveTerminalWheelZoomDelta
} from "../state/runtimeTerminalZoomShortcuts";
...
export function RuntimeTerminalPaneView({
  ...
  const { closeTerminalSession, connectTerminalSessionWebSocket, openTerminalSession } = useWorkspaceHost().api;
  ...
  useEffect(() => {
    ...
    openTerminalSession, closeTerminalSession,
    isOutputTerminal,
    paneState.id,
  ...
  }, [paneState.endpointId, paneState.id, paneState.terminalSessionId, connectTerminalSessionWebSocket]);
  ...
  }, [paneState.endpointId, paneState.terminalSessionId, closeTerminalSession]);
...
function TerminalShell({
  ...
  const { openTerminalWindow: openDetachedTerminalWindow } = useWorkspaceHost();
  const rootRef = useRef<HTMLDivElement | null>(null);
